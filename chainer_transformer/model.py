...
class TransformerDecoderUnit(Chain):
    ...
    def forward(self, inputs_encoding, outputs_unit):
        ...
        x3 = F.dropout(self.mha(inputs_encoding, inputs_encoding, x2),
                       self.p_drop)
...
class Transformer(Chain):
    ...
    def forward(self, inputs, outputs, inputs_position=0, outputs_position=0):
        ...
        inputs_positional_encoding = generate_positional_encoding(
            inputs_position, inputs_position + input_embedding.shape[-2],
            input_embedding.shape[-1])

        outputs_positional_encoding = generate_positional_encoding(
            outputs_position, outputs_position + output_embedding.shape[-2],
            output_embedding.shape[-1])
