...
class TransformerEncoderUnit(Chain):
  ...
  def forward(self, inputs_unit):
    x1 = F.dropout(
      self.mha(inputs_unit, inputs_unit, inputs_unit), self.p_drop)
...
class TransformerDecoderUnit(Chain):
  ...
  def forward(self, inputs_encoding, outputs_unit):
    ...
    x1 = F.dropout(
      self.mmha(outputs_unit, outputs_unit, outputs_unit, mask), self.p_drop)
    x2 = self.lnorm1(outputs_unit + x1)
    x3 = F.dropout(
      self.mha(inputs_encoding, inputs_encoding, x2), self.p_drop)
...
class Transformer(Chain):
  def __init__(
      self, model_dim=512, num_heads=8, encoder_depth=6, 
      decoder_depth=6, ff_dim=2048, p_drop=0.1):
    super().__init__()
    ...
    with self.init_scope():
      self.encoder = TransformerEncoder(
        encoder_depth, num_heads, model_dim, ff_dim, p_drop)
      self.decoder = TransformerDecoder(
        decoder_depth, num_heads, model_dim, ff_dim, p_drop)
      self.linear_embedding = L.Linear(None, model_dim, nobias=True)
  
  def forward(
      self, inputs, outputs, inputs_position, outputs_position):
    assert inputs.shape == outputs.shape

    inputs_positional_encoding = generate_positional_encoding(
      inputs_position, 
      inputs_position + inputs.shape[-2], 
      inputs.shape[-1])
    
    outputs_positional_encoding = generate_positional_encoding(
      outputs_position, 
      outputs_position + outputs.shape[-2], 
      outputs.shape[-1])
    ...
    inverse_embedding = F.linear(
      decoding, F.transpose(self.linear_embedding.W))
